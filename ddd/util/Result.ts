/* eslint-disable @typescript-eslint/no-explicit-any, @typescript-eslint/ban-types, @typescript-eslint/no-unused-vars, @typescript-eslint/no-empty-function, no-useless-constructor */
...
export class Result<T, E> {
  ...
  unwrapOrElse(fn: (err: E) => T): T {
    ...
    if (isUndefined(this.value)) {
      // unreachable code
      throw new Error(
        `Unexpected Result value:${String(this.value)} error:${String(
          this.error
        )}`
      );
    }
